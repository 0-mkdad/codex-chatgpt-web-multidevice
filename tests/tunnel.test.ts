...
import { TUNNEL_VERSION, connectTunnel, installTunnelClient, parseTunnelStatus, stopTunnel, tunnelClientInstallAction, tunnelCommandOutput, tunnelConnectLaunchError } from "../src/tunnel";
...
test("tunnel verification and install errors survive failed cleanup and rollback", async () => {
  for (const phase of ["verification", "installation", "upgrade"] as const) await withTunnelInstall("win32", async fixture => {
    ...
    if (phase === "upgrade") {
      ...
      fs.writeFileSync(manifest, JSON.stringify({ version: 1, tunnelClientVersion: "0.0.12",
        binarySha256: createHash("sha256").update("trusted old binary").digest("hex") }));
      fixture.verify.mockImplementation(path => ({ status: 0, stdout: path === fixture.executable ? "0.0.12" : TUNNEL_VERSION, stderr: "" }));
    }
...
test("downloads and verifies the pinned Windows x64 release asset", async () => {
  await withTunnelInstall("win32", async fixture => {
    ...
    expect(fixture.asset).toBe("tunnel-client-v0.0.14-windows-amd64.zip");
    expect(fixture.download.mock.calls.map(([url]) => String(url)).sort()).toEqual(
      [expectedAssetUrl, expectedSumsUrl].sort(),
    );
    expect(fs.readFileSync(fixture.executable)).toEqual(Buffer.from(fixture.binary));
    expect(JSON.parse(fs.readFileSync(join(fixture.root, "bin", "tunnel-client-manifest.json"), "utf8")))
      .toEqual({
        version: 1,
        tunnelClientVersion: TUNNEL_VERSION,
        asset: fixture.asset,
        archiveSha256: createHash("sha256").update(fixture.archive).digest("hex"),
        binarySha256: createHash("sha256").update(fixture.binary).digest("hex"),
      });
    expect(fixture.verify.mock.calls.some(([, args]) => args[0] === "--version")).toBe(true);
  });
});
  ...
  for (const installedVersion of ["0.0.10", "0.0.12"]) {
    await withTunnelInstall("win32", async fixture => {
      ...
      fs.writeFileSync(join(fixture.root, "bin", "tunnel-client-manifest.json"), JSON.stringify({
        version: 1,
        tunnelClientVersion: installedVersion,
        binarySha256: createHash("sha256").update(`trusted ${installedVersion} binary`).digest("hex"),
      }));
      fixture.verify.mockImplementation(path => ({
        status: 0,
        stdout: path === fixture.executable ? installedVersion : TUNNEL_VERSION,
        stderr: "",
      }));
      ...
      expect(fs.readFileSync(fixture.executable)).toEqual(Buffer.from(fixture.binary));
      const manifest = JSON.parse(fs.readFileSync(join(fixture.root, "bin", "tunnel-client-manifest.json"), "utf8"));
      expect(manifest.tunnelClientVersion).toBe(TUNNEL_VERSION);
      expect(manifest.asset).toBe(fixture.asset);
    });
  }
});

test("pins the stable tunnel-client release and migrates only trusted prior versions", () => {
...
test("connects each configured tunnel through the pinned client with file-backed credentials", () => {
  const command = spyOn(commands, "runCommand").mockReturnValue({
    status: 0,
    stdout: JSON.stringify({ running: true, healthy: true, ready: true }),
    stderr: "",
  });
  const root = fs.mkdtempSync(join(tmpdir(), "tunnel-control-plane-"));
  try {
    const settings = {
      binaryPath: join(root, "bin", "tunnel-client.exe"),
      tunnelId: `tunnel_${"a".repeat(32)}`,
      runtimeKeyFile: join(root, "secrets", "tunnel.key"),
      profileDir: join(root, "tunnel", "profiles"),
      profileName: "laptop-a",
      alias: "laptop-a",
    };
    const configValue = {
      mode: "full",
      ...
      tunnel: settings,
      runtimeCommand: ["C:\\runtime\\bun.exe", "C:\\runtime\\cli.js"],
      brokerSocketPath: "\\\\.\\pipe\\codex-chatgpt-web-laptop-a",
    } as config.AppConfig;

    connectTunnel(configValue);

    const [binaryPath, args] = command.mock.calls[0]!;
    expect(binaryPath).toBe(settings.binaryPath);
    expect(args).toContain(settings.alias);
    expect(args).toContain(settings.profileName);
    expect(args).toContain(settings.tunnelId);
    expect(args).toContain(`file:${settings.runtimeKeyFile}`);
    expect(args).toContain("--mcp-command");
    expect(args.at(-1)).toBe("--json");
    expect(command.mock.calls[0]![2]).toMatchObject({ timeout: 120_000 });
  } finally {
    command.mockRestore();
