...
export function parseTunnelStatus(output: string, alias: string, exitStatus = 0): TunnelRuntimeStatus {
  ...
  try {
    ...
    // Managed-runtime inventory derives these states from the live process and local healthz/readyz
    // probes. Control Plane polling is separate and must not be inferred from local readiness.
